...
public class InstructorRepository(AppDbContext context) : IInstructorRepository
{
    ...
    public async Task<Instructor> UpdateAsync(Instructor entity)
    {
        ...
        {
            // Se o objeto não for encontrado no banco da dados com base no Id, lança a exceção
            if (await context.Instructors.FirstOrDefaultAsync(x => x.Id == entity.Id) is null)
                throw new Exception($"O registro não foi encontrado no banco da dados");
